...
function createCategory(req, res) {
    ...
    if (params.nombre) {
        ...
        Category.find({$or:[
            {nombre: CategoryModel.nombre.toLowerCase()},{descripcion: CategoryModel.descripcion.toLowerCase()}
        ]}).exec((err, nombres)=>{
            ...
            if(nombres && nombres.length>=1){
                ...
                CategoryModel.save((err,categoryGuardado)=>{
                        if(err) return res.status(500).send({mesagge: 'Error en guardar la categoria'})
                        
                        if(categoryGuardado){
                            res.status(200).send({user: categoryGuardado})
                        }else{
